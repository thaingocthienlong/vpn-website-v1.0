...
export class SnapshotManager {
    constructor() {
        ...
        this.recentlyRemoved = new Set(); // Tracks codebases removed since last save
 ...
 */
    ...
    acquireLock(maxRetries = 5, retryInterval = 100) {
    ...
    mergeExternalEntry(codebasePath, info) {
        if (this.codebaseInfoMap.has(codebasePath))
            return; // we already know about it
        if (this.recentlyRemoved.has(codebasePath))
            return; // explicitly removed, don't re-add from disk
        this.codebaseInfoMap.set(codebasePath, info);
        if (info.status === 'indexed') {
            if (!this.indexedCodebases.includes(codebasePath)) {
                this.indexedCodebases.push(codebasePath);
                ...
                if (fs.existsSync(this.snapshotFilePath)) {
                    ...
                    if (this.isV2Format(diskSnapshot)) {
                        for (const [diskPath, diskInfo] of Object.entries(diskSnapshot.codebases)) {
                            this.mergeExternalEntry(diskPath, diskInfo);
                        }
                    }
                }
            }
