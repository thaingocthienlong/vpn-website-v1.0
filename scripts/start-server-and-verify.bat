...
echo [1/4] Checking if a process is already listening on port %PORT%...
set "FOUND_PID="
set "LAST_PID="
...
echo [2/4] Starting Next.js dev server with webpack...
if exist "%LOG_FILE%" del /f /q "%LOG_FILE%" >nul 2>&1
if exist "%ERR_FILE%" del /f /q "%ERR_FILE%" >nul 2>&1

start "next-dev-webpack" cmd /c "cd /d \"%WORKDIR%\" && npx next dev --webpack > \"%LOG_FILE%\" 2> \"%ERR_FILE%\""

echo [3/4] Confirming browser access with Playwright (auto-retry until ready)...
