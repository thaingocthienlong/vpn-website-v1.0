...
interface ScrollListProps<T> {
  ...
  itemHeight?: number | "auto"; // Optional: Fixed height for each item in pixels or auto. Defaults to 155px.
  itemClassName?: string; // Optional custom Tailwind classes for the wrapper item
}
...
const ScrollList = <T,>({
  ...
  return (
    <div
      ...
      className="scroll-list__wrp mx-auto w-full [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
      ...
      {data.map((item, index) => {
        ...
        return (
          <motion.div
            ...
            // Use a dynamic class string. If itemClassName is provided, prepend base classes.
            className={`scroll-list__item mx-auto ${itemClassName || 'max-w-3xl'}`}
            ...
            style={{
              height: itemHeight === "auto" ? "auto" : (itemHeight ? `${itemHeight}px` : "auto"),
            }}
          >
